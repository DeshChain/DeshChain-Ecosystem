...
// CreateUrbanPensionScheme creates a new urban pension scheme
func (k Keeper) CreateUrbanPensionScheme(
	ctx sdk.Context,
	contributorAddr sdk.AccAddress,
	cityCode string,
	referrerAddr sdk.AccAddress,
) (*types.UrbanPensionScheme, error) {
	// Generate scheme ID
	...
	// Create scheme
	scheme := types.UrbanPensionScheme{
		SchemeID:            schemeID,
		AccountID:           accountID,
		ContributorAddress:  contributorAddr,
		MonthlyContribution: monthlyContribution,
		ContributionPeriod:  types.UrbanPensionContributionPeriod,
		TotalContributions:  sdk.NewCoin("unamo", sdk.ZeroInt()),
		StartDate:           ctx.BlockTime(),
		MaturityDate:        ctx.BlockTime().AddDate(0, types.UrbanPensionMaturityMonth, 0),
	...
	// Store scheme
	k.SetUrbanPensionScheme(ctx, scheme)
...
func (k Keeper) ProcessMonthlyContribution(
	...
	// Get scheme
	scheme, found := k.GetUrbanPensionScheme(ctx, schemeID)
	if !found {
	...
	k.moneyOrderKeeper.Hooks().AfterPensionContribution(
...
// ProcessPensionMaturity processes pension maturity and payout
func (k Keeper) ProcessPensionMaturity(
	ctx sdk.Context,
	schemeID string,
) error {
	// Get scheme
...
func (k Keeper) addToUrbanPool(ctx sdk.Context, poolID uint64, amount sdk.Coin) error {
	...
	pool.PensionReserve = pool.PensionReserve.Add(sdk.NewCoin(amount.Denom, pensionAlloc))
...
func (k Keeper) updatePerformanceScore(ctx sdk.Context, scheme *types.UrbanPensionScheme) {
...
func (k Keeper) processInsurancePremium(ctx sdk.Context, scheme *types.UrbanPensionScheme) {
...
func (k Keeper) updateInsurancePolicy(ctx sdk.Context, scheme *types.UrbanPensionScheme) {
...
// Storage functions will be implemented here (SetUrbanPensionScheme, GetUrbanPensionScheme, etc.)