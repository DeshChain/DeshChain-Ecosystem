...
func (k Keeper) CreateScheme(ctx sdk.Context, scheme types.PensionScheme) error {
	// Validate scheme
...
func (k Keeper) SetScheme(ctx sdk.Context, scheme types.PensionScheme) {
	store := ctx.KVStore(k.storeKey)
	bz := k.cdc.MustMarshal(&scheme)
	store.Set(types.PensionSchemePrefix.Bytes(append([]byte(scheme.SchemeID)), bz)
}

// GetScheme retrieves a pension scheme by ID
func (k Keeper) GetScheme(ctx sdk.Context, schemeID string) (types.PensionScheme, bool) {
	store := ctx.KVStore(k.storeKey)
	bz := store.Get(types.PensionSchemePrefix.Bytes(append([]byte(schemeID)))
	if bz == nil {
		return types.PensionScheme{}, false
	}

	var scheme types.PensionScheme
...
func (k Keeper) HasScheme(ctx sdk.Context, schemeID string) bool {
	store := ctx.KVStore(k.storeKey)
	return store.Has(types.PensionSchemePrefix.Bytes(append([]byte(schemeID)))
}

// GetAllSchemes returns all pension schemes
func (k Keeper) GetAllSchemes(ctx sdk.Context) []types.PensionScheme {
	var schemes []types.PensionScheme
...
func (k Keeper) IterateSchemes(ctx sdk.Context, cb func(types.PensionScheme) bool) {
	store := ctx.KVStore(k.storeKey)
	iterator := sdk.KVStorePrefixIterator(store, types.PensionSchemePrefix)
...
func (k Keeper) GetActiveSchemes(ctx sdk.Context) []types.PensionScheme {
	var schemes []types.PensionScheme
	k.IterateSchemes(ctx, func(scheme types.PensionScheme) bool {
