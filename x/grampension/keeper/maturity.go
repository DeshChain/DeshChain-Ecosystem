...
func (k Keeper) ProcessMaturity(ctx sdk.Context, participantID string) error {
	...
	maturity := types.PensionMaturity{
...
func (k Keeper) SetMaturity(ctx sdk.Context, maturity types.PensionMaturity) {
...
func (k Keeper) GetMaturity(ctx sdk.Context, maturityID string) (types.PensionMaturity, bool) {
	store := ctx.KVStore(k.storeKey)
	bz := store.Get(types.MaturityProjectionPrefix.Bytes(append([]byte(maturityID)))
	if bz == nil {
		return types.PensionMaturity{}, false
	}

	var maturity types.PensionMaturity
	k.cdc.MustUnmarshal(bz, &maturity)
	return maturity, true
}

// GetUpcomingMaturities gets participants maturing in the next N days
func (k Keeper) GetUpcomingMaturities(ctx sdk.Context, days int) []types.PensionParticipant {
