...
func (k Keeper) AddPensionLiquidity(
	...
	usableLiquidity := pensionContribution.Amount.ToDec().Mul(config.LiquidityUtilizationRate).TruncateInt()
	reserveAmount := pensionContribution.Amount.Sub(usableLiquidity)

	// Create liquidity coin
	liquidityCoin := sdk.NewCoin(pensionContribution.Denom, usableLiquidity)
	reserveCoin := sdk.NewCoin(pensionContribution.Denom, reserveAmount)
	...
	k.SetPensionLiquidity(ctx, PensionLiquidity{
...
// GetPensionLiquidityUtilization returns current utilization stats
func (k Keeper) GetPensionLiquidityUtilization(ctx sdk.Context, villagePoolId uint64) PensionLiquidityStats {
...
func (k Keeper) SetPensionLiquidity(ctx sdk.Context, pl PensionLiquidity) {
...
func (k Keeper) GetPensionLiquidity(ctx sdk.Context, pensionAccountId string, poolId uint64) (PensionLiquidity, bool) {
	store := ctx.KVStore(k.storeKey)
