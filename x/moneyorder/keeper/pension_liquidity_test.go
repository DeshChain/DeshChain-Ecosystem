...
func (suite *PensionLiquidityTestSuite) TestAddPensionLiquidity() {
	...
	tests := []struct {
	...
	}{
		{
			...
			villagePoolId:       1,
			pensionContribution: sdk.NewCoin("unamo", sdk.NewInt(1000000)), // 1000 NAMO
		...
		},
		{
			...
			villagePoolId:       1,
			pensionContribution: sdk.NewCoin("unamo", sdk.NewInt(10000000)), // 10000 NAMO
		...
		},
		{
			...
			villagePoolId:       999,
			pensionContribution: sdk.NewCoin("unamo", sdk.NewInt(1000000)),
	...
	for _, tc := range tests {
		suite.Run(tc.name, func() {
			...
			if tc.shouldError {
				...
				pl, found := suite.keeper.GetPensionLiquidity(suite.ctx, "PENSION001", tc.villagePoolId)
...
func (suite *PensionLiquidityTestSuite) TestRotatePensionLiquidity() {
	...
	pl, found := suite.keeper.GetPensionLiquidity(futureCtx, "PENSION001", 1)
	suite.Require().True(found)
...
func (suite *PensionLiquidityTestSuite) TestGetPensionLiquidityUtilization() {
	...
	stats := suite.keeper.GetPensionLiquidityUtilization(suite.ctx, poolId)
...
func (suite *PensionLiquidityTestSuite) TestMonthlyRotationCycle() {
	...
	pl, found := suite.keeper.GetPensionLiquidity(maturityCtx, "PENSION-M00", poolId)
	suite.Require().True(found)
	suite.Require().False(pl.IsActive)
	
	// Verify later contributions are still active
	pl, found = suite.keeper.GetPensionLiquidity(maturityCtx, "PENSION-M11", poolId)
	suite.Require().True(found)
	suite.Require().True(pl.IsActive)
	
	// Get stats to verify rotating liquidity
	stats := suite.keeper.GetPensionLiquidityUtilization(maturityCtx, poolId)
...
func (suite *PensionLiquidityTestSuite) TestEmergencyWithdrawal() {
	...
	pl, found := suite.keeper.GetPensionLiquidity(earlyCtx, "PENSION001", 1)
	suite.Require().True(found)
	suite.Require().True(pl.IsActive)
